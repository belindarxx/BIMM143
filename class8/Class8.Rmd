...
- k=2 means group these number into 2 clusters
- nstart= 20 means run the iritation 20 times
- the plot will show the best clustering
```{r}
...
Try to get stuff out of the k, how to access it
- using $ sign
...
hc
```
The above is not a helpful print.. cuz just tell you how many points you have 
...
How many rows/colums? What is the dimension of the data? How many genes (the columns)?
```{r}
dim(mydata)
nrow(mydata)
```
...
---> This -1 function is dangerous; -1 column each time you run it
```{r}
...
HINT: Try setting beside=FALSE in your barplot() code. Why would leaving this argument out have the same effect as setting it to FALSE? See the help page with ?barplot for further details.
...
Even relatively small datasets can prove challanging to interpertate Given that it is quite difficult to make sense of even this relatively small data set. Hopefully, we can clearly see that a powerful analytical method is absolutely necessary if we wish to observe trends and patterns in larger datasets.
...
Summary PC
- shows how many PC in PC1, PC2 etc.. 
```{r}
...
- Fresh_potatoes and soft drinks predominate! 
```{r}
# Lets focus on PC2 
par(mar=c(10, 3, 0.35, 0))
# global function that adjest the margin
